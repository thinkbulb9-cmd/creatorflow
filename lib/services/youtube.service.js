...
export async function ensureValidToken(integration) {
  /**
   * Ensure access token is valid, refresh if expired
   */
  const { access_token, refresh_token, expires_at, config_json } = integration;
  const clientId = config_json?.client_id;
  const clientSecret = config_json?.client_secret;
  
  // Check if token exists
  if (!access_token) {
    throw new Error('No access token found. Please reconnect YouTube.');
  }
  
  // Check if token is expired (with 5 minute buffer)
  const now = Date.now();
  const expiresAt = expires_at || config_json?.expires_at;
  
  if (!expiresAt || now >= (expiresAt - 300000)) {
    console.log('[YouTube OAuth] Token expired or expiring soon, refreshing...');
    
    if (!refresh_token && !config_json?.refresh_token) {
      throw new Error('No refresh token found. Please reconnect YouTube.');
    }
    
    if (!clientId || !clientSecret) {
      throw new Error('YouTube OAuth credentials incomplete. Please reconnect.');
    }
  ...
  return {
    access_token: access_token || config_json.access_token,
    valid: true
  };
}
